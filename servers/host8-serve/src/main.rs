...
fn render_task_manager() -> String {
    ...
    for (service, port, runtime) in TASK_MANAGER_PORTS {
        ...
        if is_running && runtime.contains("rust") {
            rust_running += 1;
        }
    ...
    let mut out = format!(
        ...
        hbp_escape(ASOLARIA_TASK_MANAGER_PID),
...
fn render_fischer_converters(room: &Room) -> String {
    let mut out = format!(
    ...
    for (name, status, meaning) in FISCHER_CONVERTERS {
        let pid = sha16(&format!("fischer-converter|{}|{}", room.host_handle8, name));
        out.push_str(&format!(
...
fn handle_client(
    ...
    let (status, body) = match path {
        ...
        "/task-manager.hbp" => ("200 OK", render_task_manager()),
        "/unified-pipe.hbp" => ("200 OK", render_unified_pipe()),
        "/fischer-converters.hbp" => ("200 OK", render_fischer_converters(room)),
        "/summon.hbp" => {
...
mod tests {
    ...
    fn unified_pipe_and_fischer_converters_are_hbp_descriptors() {
        ...
        let converters = render_fischer_converters(&default_room());
